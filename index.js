jQuery(() => {
    ...
    getContainer().append(
        ...
        '<div class="inline-drawer-content">' +
            ...
            '<div id="bny-probe-result" style="padding:5px 0;font-size:10px;color:#666;white-space:pre-wrap;word-break:break-all;max-height:200px;overflow-y:auto;"></div>' +
    ...
    $("#bny-probe").on("click", function () {
        ...
        } catch (e) { info += "Context:读取失败\n\n"; }
        try {
            if (typeof power_user !== "undefined") {
                info += "=== power_user存在 ===\n";
                if (power_user.prompt_manager) {
                    info += "prompt_manager keys: " + Object.keys(power_user.prompt_manager).join(", ") + "\n\n";
                } else {
        ...
        } catch (e) { info += "power_user: 读取失败\n\n"; }
        try {
            if (typeof promptManager !== "undefined") {
                info += "=== promptManager全局变量存在 ===\n";
                info += "keys: " + Object.keys(promptManager).join(", ") + "\n\n";
            } else {
                info += "promptManager全局变量: 不存在\n\n";
            }
        } catch (e) { info += "promptManager: 读取失败\n\n"; }
        try {
            var prompts = document.querySelectorAll(".prompt_manager_prompt");
            info += "=== DOM探测 ===\n";
            info += "prompt条目数量: " + prompts.length + "\n";
            if (prompts.length > 0) {
                for (var i = 0; i < Math.min(prompts.length, 5); i++) {
                    var name = prompts[i].querySelector(".prompt_manager_prompt_name");
                    var toggle = prompts[i].querySelector("input[type=checkbox]");
                    info += "  " + (i + 1) + ". " + (name ? name.textContent.trim() : "无名") + " | " + (toggle ? (toggle.checked ? "开" : "关") : "无开关") + "\n";
                }
                if (prompts.length > 5) info += "  ...还有" + (prompts.length - 5) + "条\n";
            }
        } catch (e) { info += "DOM探测失败\n"; }
        r.text(info || "什么都没探测到");
    });
