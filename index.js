jQuery(() => {
    ...
    function fuzzyFind(text, pieces) {
        ...
        for (var h = 0; h < allHits.length; h++) {
            ...
            if (!best || score > best.score || (score === best.score && (segEnd - segStart) < (best.end - best.start))) {
    ...
    styleEl.textContent =
        ...
        ".box .blabel{font-size:11px;color:#999;margin-bottom:6px;display:flex;align-items:center;gap:4px;flex-wrap:wrap;}" +
        ".box .bstatus{font-size:11px;padding:2px 8px;border-radius:8px;color:#fff;display:inline-block;}" +
        ...
        ".pieces span.hit{background:#d1fae5;color:#059669;}" +
        ".pieces span.miss{background:#fee2e2;color:#dc2626;}" +
    ...
    function runAnalysis(idx) {
        ...
        for (var pi = 0; pi < pieces.length; pi++) {
            if (tLow.indexOf(pieces[pi].toLowerCase()) !== -1) { hitPieces.push(pieces[pi]); } else { missPieces.push(pieces[pi]); }
        }

        var statusHtml = "";
        if (directMatch) {
            statusHtml = '<span class="bstatus ok">✅ 正则匹配成功，无需修复</span>';
        } else if (found && found.score >= Math.ceil(found.total * 0.3)) {
            statusHtml = '<span class="bstatus warn">⚠️ 未匹配，找到相似段落（' + found.score + '/' + found.total + '碎片命中）</span>';
        } else {
            statusHtml = '<span class="bstatus no">❌ 未找到相似内容</span>';
        }

        var piecesHtml = '<div class="blabel">🧩 骨架碎片（绿=命中 红=缺失）：</div><div class="pieces">';
        for (var hi = 0; hi < Math.min(hitPieces.length, 15); hi++) {
            var hEsc = document.createElement("span");
            hEsc.className = "hit";
            hEsc.textContent = "✓ " + hitPieces[hi];
            piecesHtml += hEsc.outerHTML;
        }
        for (var mi = 0; mi < Math.min(missPieces.length, 10); mi++) {
            var mEsc = document.createElement("span");
            mEsc.className = "miss";
            mEsc.textContent = "✗ " + missPieces[mi];
            piecesHtml += mEsc.outerHTML;
        }
        piecesHtml += '</div>';
        if (missPieces.length > 0) {
            piecesHtml += '<div style="font-size:10px;color:#dc2626;padding:4px 0;">💡 红色碎片就是正文里写错/缺失的部分！</div>';
        }
        ...
        contentEl.innerHTML =
            '<div class="box"><div class="blabel">📊 ' + statusHtml + '</div>' + piecesHtml + '</div>' +
            '<div class="box"><div class="blabel">📍疑似问题段落' + (found ? '（碎片命中 ' + found.score + '/' + found.total + '）' : '') + '：</div>' +
            '<textarea id="bny-edit"></textarea></div>' +
            '<div class="btns">' +
        ...
        contentEl.querySelector("#bny-test").addEventListener("click", function (e) {
            ...
            var testResult = testReg ? testReg.test(edited) : false;
            if (testReg && testReg.global) testReg.lastIndex = 0;
            var resultEl = contentEl.querySelector("#bny-result");
            if (testResult) {
                var replaced = edited;
                try {
                    var repReg = parseRegex(rx.findRegex);
                    replaced = edited.replace(repReg, rx.replaceString);
                } catch (er) {}
                resultEl.innerHTML =
                    '<div class="box" style="margin-top:8px;"><div class="blabel"><span class="bstatus ok">✅ 修改后匹配成功！</span></div>' +
                    '<div class="blabel">替换后预览：</div><pre id="bny-preview"></pre></div>';
                contentEl.querySelector("#bny-preview").textContent = replaced.substring(0, 800);
            } else {
                resultEl.innerHTML =
                    '<div class="box" style="margin-top:8px;"><div class="blabel"><span class="bstatus no">❌ 修改后仍不匹配</span></div>' +
                    '<div style="font-size:10px;color:#999;padding:4px 0;">对照红色缺失碎片继续调整</div></div>';
            }msgEl.textContent = testResult ? "✅ 匹配成功" : "❌ 仍未匹配";
        });

        contentEl.querySelector("#bny-save").addEventListener("click", function (e) {
            e.stopPropagation();
            if (!found) { msgEl.textContent = "❌ 没有段落可替换"; return; }
            var edited = editEl.value;
            var newMes = fullMes.substring(0, found.start) + edited + fullMes.substring(found.end);
            if (saveMsg(lastMsg.idx, newMes)) { msgEl.textContent = "✅ 已替换并保存！"; }
            else { msgEl.textContent = "❌ 替换失败"; }
        });
