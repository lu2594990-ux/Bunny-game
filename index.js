jQuery(() => {
    ...
    getContainer().append(
        ...
        '<div class="inline-drawer-content">' +
            ...
            '<button id="bny-probe" style="padding:6px 20px;border-radius:8px;border:1px solid #ddd;background:#f0f0f0;color:#666;font-size:12px;cursor:pointer;width:100%;">🔍 探测酒馆接口</button>' +'<div id="bny-probe-result" style="padding:5px 0;font-size:10px;color:#666;white-space:pre-wrap;word-break:break-all;max-height:200px;overflow-y:auto;"></div>' +
    ...
    $("#bny-probe").on("click", function () {
        ...
        try {
            ...
            info += "=== 1. getPresetManager ===\n";
            try {
                var pm = ctx.getPresetManager();
                if (pm) {
                    info += "类型: " + typeof pm + "\n";
                    info += "keys: " + Object.keys(pm).join(", ") + "\n\n";
                } else {
                    info += "返回null\n\n";
                }
            } catch (e) { info += "调用失败: " + e.message + "\n\n"; }

            info += "=== 2. powerUserSettings ===\n";
            try {
                var pus = ctx.powerUserSettings;
                if (pus) {
                    info += "类型: " + typeof pus + "\n";
                    if (pus.prompt_manager) {
                        info += "prompt_manager存在!\n";
                        info += "pm keys: " + Object.keys(pus.prompt_manager).join(", ") + "\n\n";
                    } else {
                        info += "prompt_manager: 不存在\n\n";
                    }
                } else {
                    info += "返回null\n\n";
                }
            ...
            info += "=== 3. chatCompletionSettings ===\n";
            try {
                var ccs = ctx.chatCompletionSettings;
                if (ccs) {
                    info += "keys: " + Object.keys(ccs).join(", ") + "\n";
                    if (ccs.prompts) {
                        info += "prompts存在! 类型: " + typeof ccs.prompts + "\n";
                        if (Array.isArray(ccs.prompts)) {
                            info += "prompts数量: " + ccs.prompts.length + "\n";
                            for (var i = 0; i < Math.min(ccs.prompts.length, 8); i++) {
                                var p = ccs.prompts[i];
                                info += "  " + (i + 1) + ". " + (p.name || p.identifier || "无名");
                                if (p.enabled !== undefined) info += " [" + (p.enabled ? "开" : "关") + "]";
                                info += "\n";
                            }
                            if (ccs.prompts.length > 8) info += "  ...还有" + (ccs.prompts.length - 8) + "条\n";
                        }
                    }
                    if (ccs.prompt_order) {
                        info += "\nprompt_order存在! 类型: " + typeof ccs.prompt_order + "\n";
                        if (Array.isArray(ccs.prompt_order)) {
                            info += "order数量: " + ccs.prompt_order.length + "\n";
                        }
                    }
                    info += "\n";
                } else {
            ...
            info += "=== 4. extensionPrompts ===\n";
            try {
                var ep = ctx.extensionPrompts;
                if (ep) {
                    var epKeys = Object.keys(ep);
                    info += "数量: " + epKeys.length + "\n";
                    for (var i = 0; i < Math.min(epKeys.length, 5); i++) {
                        info += "  " + epKeys[i] + "\n";
                    }
                    if (epKeys.length > 5) info += "  ...还有" + (epKeys.length - 5) + "个\n";
                } else {
                    info += "返回null\n";
                }
            } catch (e) { info += "读取失败: " + e.message + "\n"; }
