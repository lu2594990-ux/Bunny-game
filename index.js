jQuery(() => {
    ...
    function runAnalysis(idx) {
        ...
        for (var hi = 0; hi < Math.min(hitPieces.length, 15); hi++) {
            piecesHtml += '<span class="hit">✓ ' + hitPieces[hi].replace(/</g, "&lt;") + '</span>';
        }
        for (var mi = 0; mi < Math.min(missPieces.length, 10); mi++) {
            piecesHtml += '<span class="miss">✗ ' + missPieces[mi].replace(/</g, "&lt;") + '</span>';
        }
        piecesHtml += '</div>';
        ...
        contentEl.innerHTML =
            ...
            '<textarea id="bny-edit">' + foundText.replace(/</g, "&lt;").replace(/>/g, "&gt;") + '</textarea></div>' +
            '<div class="btns">' +
        ...
        contentEl.querySelector("#bny-test").addEventListener("click", function (e) {
            ...
            if (testResult) {
                var replaced = edited;
                try { replaced = edited.replace(testReg, rx.replaceString); } catch (er) {}
                resultEl.innerHTML =
                    '<div class="box" style="margin-top:8px;"><div class="blabel"><span class="bstatus ok">✅ 修改后匹配成功！</span></div>' +
                    '<div class="blabel">替换后预览：</div><pre>' + replaced.replace(/</g, "&lt;").replace(/>/g, "&gt;").substring(0, 800) + '</pre></div>';
            } else {
                resultEl.innerHTML =
