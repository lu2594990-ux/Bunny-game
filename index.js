jQuery(() => {
    ...
    function setEnabled(identifier, val) {
        ...
        try {
            ...
            var domItems = document.querySelectorAll('#completion_prompt_manager .prompt_manager_prompt');
            for (var j = 0; j < domItems.length; j++) {
                var nameEl = domItems[j].querySelector('.prompt_manager_prompt_name');
                var toggle = domItems[j].querySelector('input[type="checkbox"], .toggle_prompt');
                if (!nameEl || !toggle) continue;
                var p = getPromptById(identifier);
                if (!p) continue;
                var itemName = nameEl.textContent.trim();
                if (itemName === p.name) {
                    if (toggle.type === 'checkbox' && toggle.checked !== val) {
                        toggle.click();
                    }
                    break;
                }
    ...
    styleEl.textContent =
        ...
        ".pi .mk{font-size:10px;color:#aaa;background:#f0f0f0;padding:1px 6px;border-radius:8px;flex-shrink:0;}" +
    ...
    function renderPresets() {
        ...
        for (var i = 0; i < order.length; i++) {
            (function (entry) {
                ...
                if (isMarker) {
                    var mkTag = document.createElement("span");
                    mkTag.className = "mk";
                    mkTag.textContent = "系统";
                    row.appendChild(nameEl);
                    row.appendChild(mkTag);
                }
        ...
        if (shown === 0) {
            plEl.innerHTML = '<div style="text-align:center;padding:20px;color:#ccc;font-size:12px;">没有找到条目，请先打开角色聊天</div>';
        }renderGroups();
    }
    ...
    pfInput.addEventListener("keypress", function (e) { e.stopPropagation(); });refBtn.addEventListener("click", function (e) { e.stopPropagation(); renderPresets(); pmsgEl.textContent = "🔄 已刷新"; pmsgEl.style.color = "#888"; });
    ...
    panel.addEventListener("touchend", function (e) { e.stopPropagation(); });panel.addEventListener("click", function (e) { e.stopPropagation(); });
