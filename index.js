jQuery(() => {
    ...
    styleEl.textContent =
        ...
        ".pbar{display:flex;gap:4px;padding:6px 10px;background:#fff;border-top:1px solid #fde2e8;flex-shrink:0;}" +
    ...
    panel.innerHTML =
        ...
        '<div class="tc on" data-t="search">' +
            '<div class="sh"><span class="etag"></span><input type="text" class="si" placeholder="搜索或输入网址..."/><button class="btn bgo sbtn">搜索</button><button class="btn bcl cbtn">清空</button></div>' +
        ...
        '</div>' +
        '<div class="tc" data-t="preset"><div class="pc">' +
            '<div class="pf"><input type="text" class="pfin" placeholder="🔍 筛选条目名称..."/><button class="btn bcl refbtn">刷新</button></div>' +
            '<div class="gl"></div><div class="pl"></div><div class="pmsg"></div>' +
            '<div class="pbar"><button class="pbtn sel selbtn">☑ 多选</button><button class="pbtn save savbtn">💾 存为组</button></div>' +
        '</div></div>';
    ...
    var panelOpen = false, hasSearch = false, selectMode = false, selected = {};
    ...
    function renderGroups() {
        var groups = loadGroups(); glEl.innerHTML = "";
        if (groups.length === 0) { glEl.innerHTML = '<span style="color:#ccc;font-size:11px;">多选条目后点💾存为组</span>'; return; }
        for (var i = 0; i < groups.length; i++) {
            (function (idx) {
                var g = groups[idx];
                var gc = document.createElement("div"); gc.className = "gc";
                var gn = document.createElement("span"); gn.className = "gn"; gn.textContent = g.name; gn.title = g.name + " (" + g.ids.length + "条)";
                var ga = document.createElement("button"); ga.className = "ga"; ga.textContent = "开";
                ga.addEventListener("click", function (e) { e.stopPropagation();
                    for (var j = 0; j < g.ids.length; j++) { if (!isItemEnabled(g.ids[j])) toggleItem(g.ids[j]); }
                    setTimeout(function () { renderPresets(); }, 100);
                    pmsgEl.textContent = "✅ 已开启「" + g.name + "」"; pmsgEl.style.color = "#27ae60";
                });
                var goff = document.createElement("button"); goff.className = "goff"; goff.textContent = "关";
                goff.addEventListener("click", function (e) { e.stopPropagation();
                    for (var j = 0; j < g.ids.length; j++) { if (isItemEnabled(g.ids[j])) toggleItem(g.ids[j]); }
                    setTimeout(function () { renderPresets(); }, 100);
                    pmsgEl.textContent = "❌ 已关闭「" + g.name + "」"; pmsgEl.style.color = "#e74c3c";
                });
                var gd = document.createElement("button"); gd.className = "gd"; gd.textContent = "×";
                gd.addEventListener("click", function (e) { e.stopPropagation(); var all = loadGroups(); all.splice(idx, 1); saveGroups(all); renderGroups(); });
                gc.appendChild(gn); gc.appendChild(ga); gc.appendChild(goff); gc.appendChild(gd); glEl.appendChild(gc);
            })(i);
        }
    }
    ...
    function renderPresets() {
        ...
        for (var i = 0; i < order.length; i++) {
            (function (entry) {
                ...
                var nameEl = document.createElement("div"); nameEl.className = "pn"; nameEl.textContent = name;
                ...
                tog.addEventListener("click", function (e) {
                    ...
                    setTimeout(function () {
                        ...
                        if (nowOn) tog.classList.add("on"); else tog.classList.remove("on");
                ...
                if (selectMode) { chk.style.display = "block"; tog.style.display = "none"; }
                else { chk.style.display = "none"; tog.style.display = "block"; }
                row.appendChild(nameEl); row.appendChild(tog); row.appendChild(chk); plEl.appendChild(row); shown++;
            })(order[i]);
        }
    ...
    selBtn.addEventListener("click", function (e) {
        e.stopPropagation(); selectMode = !selectMode;
        if (selectMode) { selected = {}; selBtn.textContent = "✖ 取消"; selBtn.style.background = "#f0e0e4"; selBtn.style.color = "#c44569"; }
        else { selected = {}; selBtn.textContent = "☑ 多选"; selBtn.style.background = ""; selBtn.style.color = ""; }
        renderPresets();
    });
    savBtn.addEventListener("click", function (e) {
        ...
        var groups = loadGroups(); groups.push({ name: nm.trim(), ids: ids }); saveGroups(groups);
        selected = {}; selectMode = false; selBtn.textContent = "☑ 多选"; selBtn.style.background = ""; selBtn.style.color = "";
        renderPresets(); pmsgEl.textContent = "✅ 已保存「" + nm.trim() + "」" + ids.length + "条"; pmsgEl.style.color = "#27ae60";
    });
    ...
    fab.addEventListener("touchend", function (e) {
        ...
        var wasDragging = dragging, wasMoved = hasMoved; dragging = false; hasMoved = false;
        if (wasDragging && !wasMoved) setTimeout(function () { toggleP(); }, 50);
        if (wasDragging && wasMoved) { localStorage.setItem("bnyPosX", String(posX)); localStorage.setItem("bnyPosY", String(posY)); }
    }, { passive: false });
