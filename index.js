jQuery(() => {
    ...
    /* 关闭弹窗 */
    panel.addEventListener("click", function () {ePop.style.display = "none";
    ...
    /* 主渲染 */
    function renderAll() {
        if (manageMode) { renderManage(); return; }
        renderBar();
        renderList();
        renderActions();
    }
    ...
    function renderActions() {
        ...
        if (editingGroup !== null) {
            ...
            edSave.addEventListener("click", function (e) {
                e.stopPropagation();
                var ids = [];
                for (var ek in selected) { if (selected[ek]) ids.push(ek); }
            ...
            slSave.addEventListener("click", function (e) {
                e.stopPropagation();
                var ids = [];
                for (var sk in selected) { if (selected[sk]) ids.push(sk); }
            ...
            cgOn.addEventListener("click", function (e) {
                ...
                pmsgEl.style.color = "#888";
                batchToggle(cg.ids, true, function () { renderList(); renderBar(); });
            });
            ...
            cgOff.addEventListener("click", function (e) {
                ...
                pmsgEl.style.color = "#888";
                batchToggle(cg.ids, false, function () { renderList(); renderBar(); });
            });
            ...
            abOn.addEventListener("click", function (e) {
                ...
                batchToggle(ids, true, function () { renderList(); renderBar(); });
            });
            ...
            abOff.addEventListener("click", function (e) {
                ...
                pmsgEl.style.color = "#888";
                var ids = getActiveOrder().map(function (x) { return x.identifier; });
                batchToggle(ids, false, function () { renderList(); renderBar(); });
            });
