jQuery(() => {
    ...
    function fuzzyFind(text, pieces) {
        ...
        if (best) {
            best.start = Math.max(0, best.start - 300);
            best.end = Math.min(text.length, best.end + 100);
            best.text = text.substring(best.start, best.end);
        }
        return best;
    }

    function findWrongVersions(segText, missPieces) {
        var results = [];
        for (var pi = 0; pi < missPieces.length; pi++) {
            var correct = missPieces[pi];
            var cLow = correct.toLowerCase();
            var sLow = segText.toLowerCase();
            if (sLow.indexOf(cLow) !== -1) continue;
            var coreChars = cLow.replace(/[^a-z0-9\u4e00-\u9fff]/g, "");
            if (coreChars.length < 2) continue;
            var bestSim = 0;
            var bestMatch = null;
            var minW = Math.max(2, correct.length - 5);
            var maxW = Math.min(segText.length, correct.length + 10);
            for (var wl = minW; wl <= maxW; wl++) {
                for (var si = 0; si <= segText.length - wl; si++) {
                    var cand = segText.substring(si, si + wl);
                    var candCore = cand.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]/g, "");
                    if (candCore.length === 0) continue;
                    var overlap = 0;
                    var tempCore = coreChars.split("");
                    for (var ci = 0; ci < candCore.length; ci++) {
                        var idx = tempCore.indexOf(candCore[ci]);
                        if (idx !== -1) {
                            overlap++;
                            tempCore.splice(idx, 1);
                        }
                    }
                    var sim = overlap / Math.max(coreChars.length, candCore.length);
                    if (sim > 0.7&& sim > bestSim) {
                        bestSim = sim;
                        bestMatch = { start: si, end: si + wl, wrongText: cand, correctText: correct, sim: sim };
                    }
                }
            }
    ...
    styleEl.textContent = [
        ...
        ".pieces span{font-size:10px;padding:3px 8px;border-radius:6px;font-family:monospace;cursor:pointer;transition:transform .1s;}",
        ".pieces span:active{transform:scale(.9);}",
        ".pieces .hit{background:#d1fae5;color:#059669;}",
        ".pieces .miss{background:#fee2e2;color:#dc2626;border:1px dashed #dc2626;}",
        ...
        ".clicktip{font-size:10px;color:#999;padding:2px 0;}"].join("\n");
    ...
    function insertAtCursor(textarea, text) {
        ...
        textarea.selectionEnd = start + text.length;textarea.focus();
    }
    ...
    function runAnalysis(idx) {
        ...
        var fixes = [];
        if (found && !directMatch && missPieces.length > 0) {
            fixes = findWrongVersions(found.text, missPieces);
        }
        ...
        if (directMatch) {
            statusSpan.className = "bst ok";statusSpan.textContent = "正则匹配成功，无需修复";
        } else if (found && found.score >= Math.ceil(found.total * 0.3)) {
            statusSpan.className = "bst warn";
            statusSpan.textContent = "未匹配，找到相似段落（" + found.score + "/" + found.total + "）";
        } else {
        ...
        pLabel.textContent = "骨架碎片（点击可插入到文本框）：";
        ...
        for (var hi = 0; hi < hitPieces.length; hi++) {
            ...
            hs.className = "hit";
            hs.textContent = hitPieces[hi];hs.setAttribute("data-text", hitPieces[hi]);hs.addEventListener("click", function (e) {
                e.stopPropagation();
                if (currentEditEl) {
                    insertAtCursor(currentEditEl, this.getAttribute("data-text"));msgEl.textContent = "已插入：" + this.getAttribute("data-text");
                }
            });
        ...
        for (var mi = 0; mi < missPieces.length; mi++) {
            ...
            ms.setAttribute("data-text", missPieces[mi]);
            ms.addEventListener("click", function (e) {
                e.stopPropagation();
                if (currentEditEl) {
                    insertAtCursor(currentEditEl, this.getAttribute("data-text"));
                    msgEl.textContent = "已插入：" + this.getAttribute("data-text");
                }
            });
        ...
        if (missPieces.length > 0) {
            ...
            tip.className = "clicktip";
            tip.textContent = "红色 = 正文里缺失/写错，点击可插入正确版本到文本框";
